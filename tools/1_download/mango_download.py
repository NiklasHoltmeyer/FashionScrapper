...
def download_category(category, mango, sub_categories):
    ...
    categories_filtered = [x for x in sub_categories if includes_excludes_filter(x, includes=category["includes"],
                                                                       excludes=category["excludes"])]
    logger.debug(f"Categories: {len(categories_filtered)} | {categories_filtered}")

    responses = [dl_helper.download_images(x, IGNORE_CATEGORY_EXISTING=FORCE_RESCAN) \
                for x in tqdm(categories_filtered, desc="Download Category")]
