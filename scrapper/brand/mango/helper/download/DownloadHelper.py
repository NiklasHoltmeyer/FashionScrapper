...
class DownloadHelper:
    ...
    def download_images(self, category_url, IGNORE_CATEGORY_EXISTING=False):
        ...
        def dl_job(image):
            ...
            try:
                download_file(url=image["src"], path=path, exist_ok=True)
                return 0
            except:
                return 1
    ...
    def parse_image_urls(self, category_url, category_database, IGNORE_CATEGORY_EXISTING=False):
        ...
        if not IGNORE_CATEGORY_EXISTING and len(self.visited_db.search(where('url') == category_url)) != 0:
            self.logger.debug(f"Category Visited: {category_url}")
            return {
        ...
        if len(exceptions) > 0:
            failed_db = Json_DB(self.category_path,
                                f"{category_as_filename}_failed.json")  # Json_DB(category_path, "failed.json")
