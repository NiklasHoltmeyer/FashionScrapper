...
from utils.io import Json_DB, time_logger
from utils.list import flatten, distinct_list_of_dicts
from utils.web.dynamic import driver as d_driver
from utils.web.static import download_file
...
class Asos_DownloadHelper:
    ...
    def prepare_sub_categories(self, sub_category_jobs, IGNORE_CATEGORY_EXISTING=False):
        ...
        def load_categories(jobs):
            ...
            with Pool(_threads) as p:
                r = list(
                    tqdm(p.imap(Asos_DownloadHelper.load_category, jobs),
                         desc=f"Prepare Categories - {_threads} Threads",
            ...
            def insert_items(cat_path, entries):
                ...
                with Json_DB(db_path) as db:
                    ...
                    items = filter(lambda item:
                                   not Asos_DownloadHelper.filter_entries_by_id(db_all, item["id"]), entries)
    ...
    def download_images(self, entries_dbs):
        ...
        with Pool(_threads) as p:
            return list(
                tqdm(p.imap(Asos_DownloadHelper.download_image, download_jobs),
                     desc=f"Download Images - {_threads} Threads",
    ...
    def describe_results_alt(self):
        ...
        with Json_DB(img_info_db_path) as db, Pool(self.threads) as p:
            r = list(
                tqdm(p.imap(Asos_DownloadHelper.load_image_data, imgs), desc=f"Load IMG Info - {self.threads} Threads",
                     total=len(imgs)))
